...
class KeeneticClient:
    ...
    def _auth(self) -> bool:
        ...
        if check_auth_response.status_code == 401:
            ...
            _md5          = md5(f"{self._login}:{ndm_realm}:{self._password}".encode("utf-8")).hexdigest()
            sha           = sha256((ndm_challenge + _md5).encode("utf-8")).hexdigest()
            auth_response = self._session.post(auth_endpoint, json={"login": self._login, "password": sha})

            if auth_response.status_code == 200:
                return True
            else:
                raise ConnectionError(f"Keenetic yetkilendirme başarısız oldu. Durum: {auth_response.status_code}")

        elif check_auth_response.status_code == 200:
            return True

        raise ConnectionError(f"Yetkilendirme kontrolü başarısız oldu. Durum Bilinmiyor: ({check_auth_response.status_code})")
    ...
    def metric(self, command: str, params: Dict) -> Dict:
        if self._auth():
            ...
            raise ConnectionError("Keenetic bağlantısı yok.")
